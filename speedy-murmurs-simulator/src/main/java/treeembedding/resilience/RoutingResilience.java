...
//        		int dest = Integer.parseInt(parts[1]);
        ...
        if (debug)
          System.out.println("Stats " + i + " " + stats[0] + " " + stats[1] + " " + stats[2]);
...
//            }
  ...
  /**
   * return stats of routing: entry 0: hops of -1 if not successful, entry 1: messages entry 2:
   * #nodes contained in all routes if successful, 0 otherwise entry 3: type: 0:success (later taken
   * wrt fraction of lookups within same component), 1:success (all lookups) 2:failure due to
   * lookups between diff comp. (determined later) 3-X: type of failure (ttl expired etc.) <-
   * depends on routing algo
   */
