...
public abstract class Transformation extends ParameterList {
  ...
  /**
   * Transforms the given graph and returns the transformed version. Note that in some cases, the
   * given graph might simply be transformed and returned. Therefore, a copy of the original graph
   * should be created if it should be processed afterwards.
   *
   * @param g graph to transform
   * @return transformed graph (can be a transformed version of the old object of a new object)
   */
  public abstract Graph transform(Graph g);

  /**
   * Checks if the given graph is applicable to the transformation. E.g., to allow greedy routing
   * nodes must implement the IDNode interface.
   *
   * @param g graph to check for applicability
   * @return true of the graph is applicable for this transformation, false otherwise
   */
