...
/**
 * Implements a network generator for the so-called Erdos-Renyi random graph model G(n,M). It was
 * first described by Erdos and Renyi in their Book "On random graphs I" in 1959. The model
 * generates a network topology G(n,M) with a given number of nodes (n) and a specified number of
 * edges (M). Until the targeted number of edges in the system is reached, two nodes are selected
 * uniformly at random and an edge between them is created if it does not already exist.
 * <p>
 * http://en.wikipedia.org/wiki/Erdos-Renyi_model
 * <p>
 * The parameters are the average node degree (= 2 * edges / nodes) as well as a flag for the
 * bidirectionality of edges.
 * <p>
 * Note that in this implementation, loops are not permitted, i.e., there is no edge of the form
 * (a,a).
 *
 * @author benni
 */
