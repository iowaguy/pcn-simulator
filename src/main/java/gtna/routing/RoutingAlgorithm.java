...
/**
 * Interface that must be implemented by all routing algorithms. All methods are implemented by
 * RoutingAlgorithmImpl except for the actual routing, check for applicability, and the
 * initialization.
 *
 * @author benni
 */
public abstract class RoutingAlgorithm extends ParameterList {
  ...
  /**
   * The given source attempts to route towards the destination specified by the target identifier.
   *
   * @param graph  network graph
   * @param start  node from which the routing should start
   * @param target target identifier to be routed towards
  ...
  /**
   * Checks if this routing algorithm can be applied to the given network represented by the set of
   * nodes.
   *
   * @param nodes list of all nodes in the network
   * @return true if this routing algorithm can be applied to the network represented by the given
   *         set of nodes, false otherwise
   */
  public abstract boolean applicable(Graph graph);

  /**
   * Induces a pre-processing of the routing algorithm if required to, e.g., distribute replicas or
   * init routing tables.
   *
   * @param nodes list of all nodes contained in the network
   */
