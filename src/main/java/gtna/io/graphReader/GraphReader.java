...
public abstract class GraphReader {
  ...
  /**
   * reads a graph from the given file. properties are read from all files in the same folder that
   * start with ${filename}Config.get("GRAPH_WRITER_PROPERTY_FILE_DELIMITER").
   *
   * @param filename filename of the graph
  ...
  /**
   * reads a graph from the given file. properties are read from the files listed in properties.
   *
   * @param filename   filename of the graph
   * @param properties list of property filenames
   * @return graph with properties
   */
