...
/**
 * @author Andreas Höfer Compute a fat tree with given root and maxDegree Fat tree means the
 *         neighbors of each node are sorted using their degree and nodes are added to the spanning
 *         tree starting with the highest degree until maxdegree is reached or all unvisited
 *         neighbors are part of the spanning tree
 */
public class RestrictedFatTree extends Transformation {
  ...
  /**
   * @param root      index of the root node; root = -1 => draw the root randomly; root = -2 => root
   *                  is the highest degree node
