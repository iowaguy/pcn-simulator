...
/**
 * Implements a network generator that reads an existing network topology snapshot from a specified
 * file. All types of snapshots can be used which are supported by the GraphReader class. Since only
 * one file is specified, multiple calls of the generate method always result in the exact same
 * topology.
 * <p>
 * The parameters are the filename of the snapshot and the type (as defined by the GraphReader
 * class).
 *
 * @author benni
 */
