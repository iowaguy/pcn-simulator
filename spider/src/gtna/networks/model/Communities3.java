...
/**
 * Network generator for generating fully connected sub-graphs (communities) with a specified number
 * of edges between them. The array sizes gives the size of each of the sizes.length communities.
 * The double[][] p gives the probability of an edge existing between two nodes in each community
 * where p[i][j] is the probability of an edge from community i to community j (entries in p[i][i]
 * are ignored). If the flag bidirectional is true, for every edge the backwards edge is created as
 * well. Please note that it might lead to duplicate entries if p[i][j] != 0 && p[j][i] != 0 &&
 * bidirectional == true. Therefore, only fill one part of the matrix to avoid such problems.
 * <p>
 * The flag LESS_MEMORY can be set to choose a generation procedure that uses less memory but
 * requires more computation time.
 * <p>
 * The parameter "order" can be used to change the position of the nodes in the main node array of
 * the generated graph object. If it is set to SORTED_ORDER, the main array contains first all nodes
 * from the first community, then the nodes from the second community, and so on. In case the flag
 * is true, the first node is assigned to the first community, the second node to the second and so
 * on. If the communities do not have the same size, the smaller communities are simply discarded
 * after they received all their nodes. E.g., sizes = {2, 3} would lead to the communities with node
 * indices {0, 2} {1, 3, 4}. sizes = {3, 1, 3} would result in the node assignments {0, 3, 5} {1}
 * {2, 4, 6}. If order is set to RANDOM_ORDER, the nodes are assigned to the communities at random.
 *
 * @author benni
 */
