...
public abstract class GraphProperty {
  /**
   * Writes the GraphProperty into a file that should readable using the read(filename, graph)
   * method. Always use the methods .writeHeader(.) and .writeParameter(.) to write the classname,
   * key, and other parameters into the file before starting the main content.
   *
   * @param filename where to write the data
   * @param key      key of this GraphProperty (must be added to the header of the file)
   * @return true if writing was successful; false otherwise
   */
  public abstract boolean write(String filename, String key);

  /**
   * Reads the GraphProperty from a file that should be written using .write(filename, key). Always
   * use .readHeader(.) and read*(.) to read classname, key, and parameters before starting to read
   * the content.
   *
   * @param filename where to read the GraphProperty from
   * @param graph    Graph object to which the property should be added after reading it
