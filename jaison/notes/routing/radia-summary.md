...
* Encryption:
-- Any data TO X can be "encrypted" by using X's public key (E). To "decrypt" you need X's private key (D) - which only X knows.
* Digital Signatures:
-- X can "Sign" some data to be sent using its Private Key (D) and others can verify if the data is indeed from X by verifying the signature using X's public key (E).
...
* Network Protocol:
--- #Uses a modified Link State Routing algorithm.
...
#### Questions:
1. Page 8: How do "traditional networking layers" work?
